...
<body>
    ...
    <div class="header">
        <h1>REPORTE DE CONCEPTOS DE PAGO</h1>
    ...
    <div class="content">
        ...
        <div class="resumen-ejecutivo">
            <strong>Resumen Ejecutivo:</strong> El presente documento detalla la estructura del concepto de pago consultado, filtrado según el nombre y la frecuencia indicados. Incluye información operativa crucial como el valor establecido (monto), la recurrencia del cobro (frecuencia) y la tolerancia de tiempo otorgada (días de gracia).
        </div>
        ...
        <table class="data-table">
            <thead>
                <tr>
                    <th style="width: 5%;">#</th>
                    <th style="width: 35%;">Nombre del Concepto</th>
                    <th style="width: 20%;">Monto</th>
                    <th style="width: 20%;">Frecuencia</th>
                    <th style="width: 20%;">Días de Gracia</th>
                </tr>
            </thead>
            <tbody>
                <?php
                ...
                if (!empty($datos)) {
                    foreach ($datos as $r): 
                        ...
                        // Aseguramos que el monto se vea bien con dos decimales y la coma
                        $monto = number_format((float)$r['monto'], 2, ',', '.');
                        
                        // Adaptamos la letra a una palabra completa
                        $frecuencia_db = strtoupper(htmlspecialchars($r['frecuencia']));
                        $frecuencia_str = match($frecuencia_db) {
                            'L' => 'Libre',
                            'M' => 'Mensual',
                            'A' => 'Anual',
                            'U' => 'Única',
                        ...
                        $dias = ($r['dias_gracia'] == 0) ? 'No Aplica' : htmlspecialchars($r['dias_gracia']) . ' Días';
                ?>
                    <tr>
                        <td class="data-cell"><?= $id ?></td>
                        <td class="data-cell"><strong><?= $nombre ?></strong></td>
                        <td class="data-cell" style="font-weight: bold; color: #2b6cb0;"><?= $monto ?></td>
                        <td class="data-cell"><?= $frecuencia_str ?></td>
                        <td class="data-cell"><?= $dias ?></td>
                    </tr>
                <?php 
                    endforeach; 
                } 
